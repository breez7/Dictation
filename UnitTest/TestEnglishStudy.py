...
class TestEnglishStudy(unittest.TestCase):
    ...
    def testTopicManager(self):
        ...
        topic = self.topicManager.addTopic(
            TestEnglishStudy.title, TestEnglishStudy.genere, TestEnglishStudy.path, TestEnglishStudy.script)
        self.assertTrue(topic.id > 0)

        getTopic = self.topicManager.getTopic(topic.id)
        self.assertTrue((getTopic.title, getTopic.genere, getTopic.mediaFilePath, getTopic.script) == (
            TestEnglishStudy.title, TestEnglishStudy.genere, TestEnglishStudy.path, TestEnglishStudy.script))

        self.topicManager.analyseTopic(topic)
        self.assertTrue(topic.sttResult != None)
