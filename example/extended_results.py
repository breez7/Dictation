...
from os import path
AUDIO_FILE = path.join(path.dirname(path.realpath(__file__)), "woman1_wb.wav")
#AUDIO_FILE = path.join(path.dirname(path.realpath(__file__)), "bbc.mp3")
...
IBM_USERNAME = "12dd2d69-9bb7-45c7-8bc9-c193bd2dba63" # IBM Speech to Text usernames are strings of the form XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX
IBM_PASSWORD = "rnAnWqvZvHzt" # IBM Speech to Text passwords are mixed-case alphanumeric strings
# try:
#     print("IBM Speech to Text results:")
#     res = r.recognize_ibm(audio, username=IBM_USERNAME, password=IBM_PASSWORD, show_all=True)
#     pprint(res) # pretty-print the recognition result
# except sr.UnknownValueError:
...
res = requests.post('https://stream.watsonplatform.net/speech-to-text/api/v1/recognize?timestamps=true&word_alternatives_threshold=0.9&continuous=true', data=data, auth=(IBM_USERNAME, IBM_PASSWORD), headers={"Content-Type":"audio/wav","X-Waston-Learning-Opt-Out":"true"})
print(res.text)